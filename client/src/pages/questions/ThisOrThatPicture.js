...
const ThisOrThatPicture = ({ question, setFinalAnswers}) => {
    ...
    const toggleFocus = id => {
        const clickedRowIndex = Math.floor(answers.findIndex(answer => answer._id === id) / 2); // Index der Zeile, zu der die geklickte Antwort gehört
        const selectedAnswersInRow = focusedButtons.filter(buttonId => Math.floor(answers.findIndex(answer => answer._id === buttonId) / 2) === clickedRowIndex); // Ausgewählte Antworten in derselben Zeile wie die geklickte Antwort

        if (selectedAnswersInRow.length === 1) {
            // Wenn bereits eine Antwort in der Zeile ausgewählt ist, entferne sie
            setFocusedButtons(prevState => prevState.filter(buttonId => buttonId !== selectedAnswersInRow[0]));
        }
                        ...
                        <div className='answer-grid-thisorthatpicture'>
                            {answers.map((answer, index) => (
                                <React.Fragment key={answer._id}>
                                    ...
                                    {(index % 2 === 0) && index < answers.length - 1 && <div className="oder-text">oder</div>}
                                </React.Fragment>
                            ))}
                        </div>
                    </React.Fragment>
